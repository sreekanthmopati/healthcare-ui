...
const BulkAdmission = () => {
  ...
  // ---------------------------------------
  // Global validation effect
  // Recalculates ALL errors whenever selections or capacities change
  // ---------------------------------------
  useEffect(() => {
    const newErrors = {};

    departmentSelections.forEach(selection => {
      const deptId = selection.departmentId;
      const count = selection.count || 0;

      if (!count) {
        // If no count, no error for this dept
        return;
      }

      let error = "";

      // 1) Validate against registered count
      const maxCount = registeredCounts[deptId] || 0;
      if (maxCount === 0) {
        error = "No registered patients for this department today";
      } else if (count > maxCount) {
        error = `Cannot admit more than ${maxCount} registered patients`;
      }

      // 2) Validate ward capacity (if ward chosen and no previous error)
      if (!error && selection.wardId) {
        const wardId = selection.wardId;
        const capacity = wardBedAvailability[wardId];

        if (capacity != null) {
          const totalRequested = departmentSelections.reduce(
            (sum, item) =>
              item.wardId === wardId && item.count ? sum + item.count : sum,
            0
          );

          if (totalRequested > capacity) {
            const ward = wards.find(w => w.ward_id === Number(wardId));
            const name = ward?.ward_name || `Ward ${wardId}`;
            error = `Total requested in ${name} is ${totalRequested}, but only ${capacity} beds are available`;
          }
        }
      }

      if (error) {
        newErrors[deptId] = error;
      }
    });

    setErrors(newErrors);
  }, [departmentSelections, wardBedAvailability, registeredCounts, wards]);

  // ---------------------------------------
  // Form validity: at least one valid row with count + diagnosis + ward
  // ---------------------------------------
  useEffect(() => {
    const hasValidRow = departmentSelections.some(selection => {
      if (selection.count > 0) {
        return selection.diagnosisId && selection.wardId;
      }
      return false;
    });
    setIsFormValid(hasValidRow);
  ...
  // ✅ Only updates selection; validation handled by effect
  const handleCountChange = (deptId, value) => {
    if (value === "") {
      setDepartmentSelections(prev =>
        prev.map(item =>
          item.departmentId === deptId
            ? { ...item, count: "", diagnosisId: "", wardId: "" }
            : item
        )
      );
      return;
  ...
  // ✅ Only updates ward + capacity; validation handled by effect
  const handleWardChange = async (deptId, value) => {
    ...
    if (!value) {
      // Ward cleared: no need to fetch capacity
      return;
    }

    try {
      const capacity = await getAvailableBedsCount(value);
      setWardBedAvailability(prev => ({
        ...prev,
        [value]: capacity
      }));
  ...
  const handleSubmit = async (e) => {
    ...
    const hasErrors =
      Object.values(errors).some(msg => msg) || !isFormValid;
    ...
    const admissionsToCreate = departmentSelections
      ...
      .map(item => {
        const dept = departments.find(d => d.DepartmentID === item.departmentId);
        const diag = diagnosesMap[item.departmentId]?.find(
          d => d.DiagnosisID === parseInt(item.diagnosisId, 10)
        );
        const ward = wards.find(w => w.ward_id === parseInt(item.wardId, 10));
              ...
              {departments.map(dept => {
                ...
                return (
                  <tr key={dept.DepartmentID} className="border">
                    ...
                    <td className="py-2 px-4 border text-teal-700 font-semibold">
                      {registeredToday}
                    </td>
                    <td className="py-2 px-4 border">
                      <input
                        ...
                        value={selection?.count ?? ""}
                        onChange={e =>
                          handleCountChange(dept.DepartmentID, e.target.value)
                        }
                        className={`w-20 p-1 border rounded text-center ${
                          errors[dept.DepartmentID] ? "border-red-500" : ""
                        }`}
                        placeholder="0"
                      />
                      {errors[dept.DepartmentID] && (
                    ...
                    <td className="py-2 px-4 border">
                      {selection?.count > 0 ? (
                        <select
                          value={selection?.wardId || ""}
                          onChange={e =>
                            handleWardChange(dept.DepartmentID, e.target.value)
                          }
                          className="w-full p-1 border rounded"
                          required
                        >
                          ...
                          {wards.map(ward => (
                            <option key={ward.ward_id} value={ward.ward_id}>
                              {ward.ward_name} ({ward.ward_type}) -{" "}
                              {wardBedAvailability[ward.ward_id] ?? "..."} beds
                            </option>
                          ))}
                        </select>
                      ) : (
